 ...
 * Adapted from OpenClaw's src/memory/batch-gemini.ts
 */

import { createHash } from "node:crypto";
import { runWithConcurrency } from "../../utils/retry.js";
...
function getGeminiBaseUrl(gemini: GeminiEmbeddingClient): string {
  return gemini.baseUrl?.replace(/\/$/, "") ?? "https://generativelanguage.googleapis.com/v1beta";
}
...
function buildGeminiUploadBody(params: { jsonl: string; displayName: string }): {
  ...
  const boundary = `memorymcp-${hashText(params.displayName)}`;
