 ...
 * Adapted from OpenClaw's src/memory/batch-openai.ts
 */

import { createHash } from "node:crypto";
import { retryAsync, runWithConcurrency } from "../../utils/retry.js";
...
function getOpenAiBaseUrl(openAi: OpenAiEmbeddingClient): string {
  return openAi.baseUrl?.replace(/\/$/, "") ?? "https://api.openai.com";
}
...
async function submitOpenAiBatch(params: {
  ...
  const fileRes = await fetch(`${baseUrl}/v1/files`, {
    method: "POST",
  ...
  const batchRes = await retryAsync(
    async () => {
      const res = await fetch(`${baseUrl}/v1/batches`, {
        method: "POST",
...
async function fetchOpenAiBatchStatus(params: {
  ...
  const baseUrl = getOpenAiBaseUrl(params.openAi);
  const res = await fetch(`${baseUrl}/v1/batches/${params.batchId}`, {
    headers: getOpenAiHeaders(params.openAi, { json: true }),
  });
  if (!res.ok) {
    const text = await res.text();
...
async function fetchOpenAiFileContent(params: {
  ...
  const baseUrl = getOpenAiBaseUrl(params.openAi);
  const res = await fetch(`${baseUrl}/v1/files/${params.fileId}/content`, {
    headers: getOpenAiHeaders(params.openAi, { json: true }),
  });
  if (!res.ok) {
    const text = await res.text();
