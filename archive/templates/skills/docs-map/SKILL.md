...
**DO NOT use Bash for file counting or searching.** Bash commands like `dir`, `find`, or piped commands can escape the project directory and scan the entire system.

**ALWAYS use these tools instead:**
- `Glob` tool with explicit `path` parameter for file discovery
- `Grep` tool for content searching

```
# WRONG - can scan entire system
Bash: dir /b /s *.py | find /c ".py"
Bash: find . -name "*.ts" | wc -l

# CORRECT - scoped to project
Glob: pattern="**/*.py", path="D:\Project"
Glob: pattern="**/*.ts", path="D:\Project\src"
```

### Directory Creation (Cross-Platform)
**ALWAYS use `mkdir -p` with full absolute paths** for reliable cross-platform directory creation.

```bash
# WRONG - Windows cmd style, unreliable in mixed shells
mkdir project_docs project_docs\snapshots project_docs\codemaps

# CORRECT - works on Windows (Git Bash/WSL) and Unix
mkdir -p "/full/path/project_docs/snapshots" "/full/path/project_docs/codemaps" "/full/path/project_docs/features"
```

**Key rules:**
1. Always use forward slashes `/` or escape backslashes
2. Always provide full absolute paths
3. Use `-p` flag to create parent directories
4. Quote paths to handle spaces
...
3. **Auto-detect project info** (see Implementation Notes for tool usage):
   - Project name from package.json/Cargo.toml/go.mod/pyproject.toml (use Read tool)
   - Version from manifest or git tags
   - Git commit via `git rev-parse --short HEAD` (Bash OK for this)
   - Tech stack from dependencies and file extensions (use Glob tool, NOT Bash)
   - Directory structure from filesystem (use Glob tool, NOT Bash)
   - File counts per type (use Glob tool with explicit path, NOT Bash)
   - Quick start command from scripts (read package.json/scripts)
...
Create these directories under `project_docs/` (use `mkdir -p` with full paths - see Implementation Notes):
